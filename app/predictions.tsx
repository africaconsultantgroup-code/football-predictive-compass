...
async function loadPredictions() {
  try {
    return { predictions: await getUpcomingFootballPredictions(), failed: false } as const;
  } catch {
...
export async function PredictionsContent() {
  ...
  return <div className="grid gap-5 pt-6 lg:grid-cols-2">{predictions.map((prediction) => <PredictionCard key={prediction.prediction_id} prediction={prediction} />)}</div>;
}
